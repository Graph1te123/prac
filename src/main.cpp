...
namespace {
    ...
    struct ScopedServiceHandle {
        ...
        explicit ScopedServiceHandle(SC_HANDLE handle)
            : value(handle) {}
    ...
    StartupDecision EnsureServiceIsRunning() {
        ScopedServiceHandle serviceManager(
    ...
    bool StopServiceViaRpc() {
        ...
        return WaitForServiceState(service.get(), SERVICE_STOPPED, 30000);
    }
    ...
    std::wstring GetCurrentUserSidString() {
        ...
        if (buffer != nullptr &&
            ...
            if (ConvertSidToStringSidW(buffer->User.Sid, &sidString) &&
                sidString != nullptr) {
                result = sidString;
                LocalFree(sidString);
            }
